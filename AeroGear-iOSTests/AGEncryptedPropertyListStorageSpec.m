...
SPEC_BEGIN(AGEncryptedPropertyListStorageSpec)
    ...
    describe(@"AGEncryptedPropertyListStorage", ^{
        context(@"when newly created", ^{
            ...
            // An encrypted 'property-list' storage object:
            __block AGEncryptedPropertyListStorage *encrPlistStore = nil;
            ...
            beforeEach(^{
                ...
                encrPlistStore = [AGEncryptedPropertyListStorage storeWithConfig:config];
            });

            afterEach(^{
                // remove all elements from the store
                // so next test starts fresh
                [encrPlistStore reset:nil];
            });

            it(@"should not be nil", ^{
                [encrPlistStore shouldNotBeNil];
            });
            ...
            it(@"should fail eagerly when trying to save an object with an NSNull value", ^{
                ...
                success = [encrPlistStore save:user1 error:nil];
                [[theValue(success) should] equal:theValue(NO)];

                // should be empty
                NSArray* objects = [encrPlistStore readAll];
            ...
            it(@"should save a single object ", ^{
                ...
                BOOL success = [encrPlistStore save:user error:nil];
            ...
            it(@"should read an object _after_ storing it", ^{
                ...
                // read it
                NSMutableDictionary* object = [encrPlistStore read:@"0"];
            ...
            it(@"should read an object _after_ storing it (using readAll)", ^{
                ...
                BOOL success = [encrPlistStore save:user1 error:nil];
                [[theValue(success) should] equal:theValue(YES)];
            ...
            it(@"should read nothing out of an empty store", ^{
                ...
                [[theValue([encrPlistStore isEmpty]) should] equal:theValue(YES)];

            });

            it(@"should read not object out of an empty store", ^{
                NSMutableDictionary *object = [encrPlistStore read:@"someId"];
            ...
            it(@"should read and save multiple objects", ^{
                ...
                // store it
                BOOL success = [encrPlistStore save:users error:nil];
                [[theValue(success) should] equal:theValue(YES)];
            ...
            it(@"should not be empty after storing objects", ^{
                ...
                // store it
                [encrPlistStore save:users error:nil];
                ...
                // check if empty:
                [[theValue([encrPlistStore isEmpty]) should] equal:theValue(NO)];
            });
            ...
            it(@"should read nothing after reset", ^{
                ...
                // store it
                success = [encrPlistStore save:users error:nil];
                [[theValue(success) should] equal:theValue(YES)];

                // read it
                objects = [encrPlistStore readAll];
                [[objects should] haveCountOf:(NSUInteger)3];
                [[objects should] containObjects:user1, nil];
                [[objects should] containObjects:user2, nil];
                [[objects should] containObjects:user3, nil];
                ...
                success = [encrPlistStore reset:nil];
                [[theValue(success) should] equal:theValue(YES)];
            ...
            it(@"should not read a remove object", ^{
                ...
                success = [encrPlistStore remove:user1 error:nil];
                [[theValue(success) should] equal:theValue(YES)];
            ...
            it(@"should not remove a non-existing object", ^{
                ...
                success = [encrPlistStore remove:user2 error:nil];
                [[theValue(success) should] equal:theValue(NO)];
            ...
            it(@"should perform filtering using an NSPredicate", ^{
                ...
                results = [encrPlistStore filter:predicate];
        ...
        context(@"should fail to reload with corrupted crypto params", ^{
            ...
            beforeEach(^{
                ...
                AGPassphraseEncryptionServices *encrService = [[AGPassphraseEncryptionServices alloc] initWithConfig:cryptoConfig];
                ...
                [encrPlistStore save:user error:nil];
            });
            ...
            it(@"should fail with corrupted crypto salt on (read)", ^{
                ...
                // reload with corrupted salt
                AGPassphraseCryptoConfig *cryptoConfig = [[AGPassphraseCryptoConfig alloc] init];
                [cryptoConfig setSalt:kSaltFail];
                [cryptoConfig setPassphrase:kPassphrase];
            ...
            it(@"should fail with corrupted crypto passphrase with (read)", ^{
                ...
                // reload with corrupted passphrase
                AGPassphraseCryptoConfig *cryptoConfig = [[AGPassphraseCryptoConfig alloc] init];
                [cryptoConfig setSalt:kSalt];
